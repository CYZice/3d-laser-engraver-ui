...
int &TwoDMesh::get(size_t x, size_t y)
{
    if (x >= 4 || y >= 4)
    {
        std::cerr << "Error: TwoDMesh::get() - Invalid coordinates: "
                  << "x=" << x << ", y=" << y << std::endl;
        throw std::out_of_range("TwoDMesh coordinates out of range");
    }
    return meshData.at(y * 4 + x);
}
...
BinaryMesh::BinaryMesh(const OriginalModel &data, double resolution)
{
    ...
    m_originalMesh = Eigen::Tensor<int, 3>(xRange + 1, yRange + 1, zRange + 1); // +1 避免最大坐标点越界
    m_processingMesh = Eigen::Tensor<BasicMesh, 3>(xRange + 1, yRange + 1, zRange + 1);
...
TowDBinaryMesh::TowDBinaryMesh(const OriginalModel &data, double resolution)
{
    if (data.empty())
        return;
    ...
    if (xRange > max_dimension || yRange > max_dimension || zRange > max_dimension)
    {
        ...
        // 初始化空张量，防止后续访问时出错
        m_originalMesh = Eigen::Tensor<int, 3>(0, 0, 0);
        m_processingMesh = Eigen::Tensor<TwoDMesh, 3>(0, 0, 0);
        return;
    }
    ...
    m_originalMesh = Eigen::Tensor<int, 3>(xRange + 1, yRange + 1, zRange + 1); // +1 避免最大坐标点越界
    m_processingMesh = Eigen::Tensor<TwoDMesh, 3>(xRange + 1, yRange + 1, zRange + 1);
    m_originalMesh.setConstant(-1);
...
void TowDBinaryMesh::process()
{
    std::random_device m_rd{};
    std::mt19937 m_generator{m_rd()};
    std::uniform_int_distribution<int> m_distribution{0, 15};
    ...
    for (long long dimensionX{0}; dimensionX < m_originalMesh.dimension(0); ++dimensionX)
    {
        for (long long dimensionY{0}; dimensionY < m_originalMesh.dimension(1); ++dimensionY)
        {
            for (long long dimensionZ{0}; dimensionZ < m_originalMesh.dimension(2); ++dimensionZ)
            {
                const int stair{m_originalMesh(dimensionX, dimensionY, dimensionZ) / 16};
                if (stair <= 0)
                {
                    continue;
                }
                ...
                // 检查stair值，防止创建过大的集合
                const int max_stair = 16; // TwoDMesh::meshData的大小是16
                int actual_stair = stair;
                if (actual_stair > max_stair)
                {
                    std::cerr << "Warning: Stair value too large, clamping to max allowed." << std::endl;
...
bool TowDBinaryMesh::outputDXF(const std::string &fileName, DXFFormat format)
{
    std::ofstream file;
    if (format == DXFFormat::ASCII)
    {
    ...
    if (format == DXFFormat::ASCII)
    {
        ...
        for (long long x = 0; x < m_processingMesh.dimension(0); ++x)
        {
            for (long long y = 0; y < m_processingMesh.dimension(1); ++y)
            {
                for (long long z = 0; z < m_processingMesh.dimension(2); ++z)
                {
                    const TwoDMesh &mesh = m_processingMesh(x, y, z);
                    for (int i = 0; i < 16; ++i)
                    {
                        if (mesh.meshData[i] == 1)
                        {
                            // 将 16 个索引转换为 4x4x1 坐标
                            int subX = i % 4;
                            int subY = i / 4;
                            int subZ = 0;
...
int TowDBinaryMesh::toGray(const Eigen::Vector4f &normalizeColor)
{
    const float &colorR{normalizeColor.x()};
    const float &colorG{normalizeColor.y()};
    const float &colorB{normalizeColor.z()};
